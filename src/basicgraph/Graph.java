 ...
 * 
 * @author UCSD MOOC development team and Jun Lu
 * 
 */
...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		Collections.sort(list, new Comparator<Integer>() {
			public int compare(Integer v1, Integer v2) {
				return getNeighbors(v2.intValue()).size()+getInNeighbors(v2.intValue()).size()
						-getNeighbors(v1.intValue()).size()-getInNeighbors(v1.intValue()).size();
			}
		});
		for(int i=0; i<numVertices; ++i) {
			int degree = getNeighbors(list.get(i).intValue()).size()+getInNeighbors(list.get(i).intValue()).size();
			list.set(i, degree);
		}
		return list;
	}
	 ...
	 * XXX: Implement in part 2 for each subclass of Graph
	 */
