...
public class GraphAdjMatrix extends Graph {
	...
	public List<Integer> getDistance2(int v) {
		int[][] hopIn2Matrix = multiplicer(adjMatrix, adjMatrix);
		List<Integer> listHop2 = new ArrayList<Integer>();
		
		for(int i=0; i<hopIn2Matrix[v].length; ++i) {
			if(hopIn2Matrix[v][i] != 0) {
				for(int j=0; j<hopIn2Matrix[v][i]; ++j) {
					listHop2.add(i);
				}
			}
		}
	 ...
	 * @author Jun Lu
	 */
	public int[][] multiplicer(int[][] A, int[][] B) {
